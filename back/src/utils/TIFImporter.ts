...
export class TIFImporter {
  async importarPoligonosDoTIF(
    ...
    if (fs.existsSync(geojsonPath)) {
      ...
      // Opcional: você pode querer ler e importar o GeoJSON existente aqui se essa for a lógica desejada.

      // Se o objetivo é apenas pular a regeração do GeoJSON mas ainda importar os dados dele:

      // const geojson = JSON.parse(fs.readFileSync(geojsonPath, "utf8"));

      // ... (código de inserção duplicado ou refatorado para uma função)

      // Por ora, vamos apenas pular tudo se o GeoJSON existe.

      return;
    }

    // 1. Executa gdal_polygonize para gerar GeoJSON
    ...
    // 2. Lê o GeoJSON gerado

    const geojson = JSON.parse(fs.readFileSync(geojsonPath, "utf8"));

    // 3. Para cada polígono, insere na tabela
